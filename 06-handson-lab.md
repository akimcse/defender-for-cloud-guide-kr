[🏠 전체 목차](./README.md)　·　**Part 3 · 실습·활용**　·　페이지 7 / 9

# 06 · 핸즈온 랩 — 처음부터 끝까지 30분 실습
...
이 랩은 **실제 Azure 포털**에서 진행하는 최소 실습입니다. 각 축의 심화 데모는 [07 · 실무 활용](./07-use-cases.md)에서 다룹니다.
...
## Step 1 · Defender for Cloud 열기 (기초 CSPM 자동 활성화)

1. Azure 포털에서 **Microsoft Defender for Cloud** 검색·선택
2. 좌측 메뉴 **개요(Overview)** 로 이동

이 순간 해당 구독에 **기초 CSPM(무료)** 이 자동 활성화됩니다 — 보안 점수, 자산 인벤토리, 권장사항, MCSB 규정 준수.
...
2. 기초 CSPM(무료)은 그대로 둬도 과금되지 않습니다.
...
- **워크플로 자동화**: High 심각도 경고 발생 시 Teams/메일 알림을 보내는 Logic App 연결 (→ [02 · 핵심 개념 §9](./02-concepts.md))
- **공격 경로**: (Defender CSPM 필요) 개요 → 공격 경로 분석에서 인터넷 노출→핵심 자산 경로 탐색

각 축의 실제 운영 시나리오는 [07 · 실무 활용](./07-use-cases.md)에서 스토리로 이어집니다.
...
| ◀ 이전 | ▶ 다음 |
| :-- | --: |
| [02 · 핵심 개념](./02-concepts.md) | [07 · 실무 활용](./07-use-cases.md) |
